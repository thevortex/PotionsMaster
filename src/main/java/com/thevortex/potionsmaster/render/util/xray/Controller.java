...
public class Controller {
	...
	private static boolean playerHasMoved() {
		if ((Minecraft.getInstance().level == null) && (Controller.drawOres)) {
			toggleDrawOres();
			return false;
		}
	...
	public static synchronized void requestBlockFinder(boolean force) {
		if (drawOres() && (task == null || task.isDone()) && (force || playerHasMoved())) // world/player check done by drawOres()
		{
			...
			//PotionsMaster.LOGGER.info("min " + Minecraft.getInstance().level.getMinBuildHeight() + " >> 4 = "+ (Minecraft.getInstance().level.getMinBuildHeight() >> 4) + "max " + Minecraft.getInstance().level.getMaxBuildHeight() + " >> 4 = "+ (Minecraft.getInstance().level.getMaxBuildHeight() >> 4)  );
